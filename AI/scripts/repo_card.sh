...
#   --mongo --vercel --dns --status --level ok|warn|error|unknown --by
#
...
ARGS=$(python3 - "$NAME" "$DESC" "$GROUP" "$LOCAL" "$APP" "$API" "$MONGO" "$VERCEL" "$DNS" "$STATUS" "$LEVEL" "$BY" "$CARD_JSON" <<'PY'
import json, os, sys
name,desc,group,local,app,api,mongo,vercel,dns,status,level,by,card_json = sys.argv[1:14]
out = {"repoName": name}
# 1) static file
