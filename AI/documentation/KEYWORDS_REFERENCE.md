...
| `schedule plan` | **MYTHOS-GRADE PLAN + 10-DAY SCHEDULE (planning only — DO NOT BUILD).** The standing protocol to make this app world-class + production-grade. **(1) Produce the plan:** read `AI/plan/*`, `AI/state/STATE.md`, handoff, README, source layout + TODOs, then write `AI/plan/MYTHOS_IMPROVEMENT_PLAN.md` — a comprehensive, ambitious improvement & feature plan covering **tooling, tracking, service & integration, functionality, UX/web design, user journey, and the most practical use cases**. Be opinionated about great design + real user value. **(2) Set the 10-day schedule (shows on dashboard `/plan`):** call the master gateway `plan_set` MCP tool (`http://localhost:3100/mcp`) with `repo`, `startDate` (today), `days:[{day,focus,status:"enabled"}]` — one focus/day for 10 days (fires auto-compute to 23:00 UTC ≈ 9am Sydney). **(3) Schedule the work:** for each day create session-sized tasks (~60–150 min, atomic, commit on `test`) via `./AI/scripts/schedule_task.sh` — categorized REFACTOR / FUNCTIONALITY / UIUX / PLAN-COMPLETION / QUALITY, with specialist + priority + `--model claude-fable-5`. **(4) DO NOT build** — planning + scheduling only; the CLI runner executes across later sessions. Exclude credential/user-blocked items. Commit the plan doc on `test`. |
