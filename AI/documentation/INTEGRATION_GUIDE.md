...
- **Global vs. Local:** The rules in `AI/AI_RULES.md` from the master template are **Global Mandates**. If the existing repo has conflicting tech stack rules, they must be updated to align with the global standard (Docker, Next.js, MongoDB, etc.) unless an explicit architectural exception is documented.
- **Append, Don't Overwrite:** For logs (`claude.md`, `gemini.md`, `copilot.md`) and `STATE.md`, always append existing history to the new standardized format.
...
1.  **Combine Logs:** If `claude.md`, `gemini.md`, or `copilot.md` were found anywhere in the workspace, copy their historical entries and paste them under the appropriate headers in the corresponding log files inside the `AI/` directory.
2.  **Initialize STATE.md:** If there are existing "status", "progress", or "session" files, summarize them and move the content into `AI/STATE.md` at the workspace root to build rich context for the next agent.
...
> 4. **Standardize:** Apply the `AI/AI_RULES.md` global mandates. If project-specific rules were found, move them to a new 'Project-Specific Overrides' section in `AI/AI_RULES.md`.
...
> 6. **Log:** Document the migration steps taken in your respective log (e.g., `AI/gemini.md`)."
